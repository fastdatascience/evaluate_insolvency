...
SUPPORTED_MODELS = {'qwen-plus', 'qwen3.5-plus'}
...
if os.environ.get("DASHSCOPE_API_KEY") == "" or os.environ.get("DASHSCOPE_API_KEY") is None:
    print("Please set environment variable DASHSCOPE_API_KEY first.")
    exit()
...
for idx in range(len(df)):
    ...
    for attempt in range(3):
        print("attempt calling Qwen API:", attempt)
        try:
            response = requests.post('https://dashscope-intl.aliyuncs.com/compatible-mode/v1/chat/completions', headers=headers, json=json_data)
